  ...
  if(str_contains($status, '1'))
  {
      ...
      if($estado_actual==$estado_anterior){
        ...
        if((strlen($numero_de_placa_asignar))>0){
          ...
          $queriesa= "UPDATE slots SET timestamp_cambio_ocupado='$now',auto_slot_img='$fotografia_entrada',placa_slot='$numero_de_placa_asignar' WHERE id_slot='$id_slot' AND id_parqueo='$id_park_slot'";
          $resultadosa = pg_query($conn, $queriesa) or die('ERROR : ' . pg_last_error());
          pg_free_result($resultadosa);
          ...
          $query = "DELETE FROM  from por_asignar_espacio WHERE id_parqueo='$id_park_slot' AND id_asignar='$id_asignar'";
